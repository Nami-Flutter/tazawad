...
Future getProductMetaData(int? productId) async {
  if (!await isGuestUser() && await isLoggedIn()) {
//wc/v3/products?&consumer_key=ck_19007b0c3b317748d08d815db1f9ac161e7b931b&consumer_secret=cs_1936066ce1273df0780dfcb822d29bd17640ea88&product_id=1509

    return handleResponse(await MightyAPI().getAsync(
        'wc/v3/products?&consumer_key=ck_19007b0c3b317748d08d815db1f9ac161e7b931b&consumer_secret=cs_1936066ce1273df0780dfcb822d29bd17640ea88&product_id=$productId',
        requireToken: false ));
...
Future getCustomer(id) async {
  return handleResponse(await MightyAPI().
  getAsync('wc/v3/customers/$id'));
}
...
Future getMembershopPlan(request) async {
  if (!await isGuestUser() && await isLoggedIn()) {
    return handleResponse(await
     MightyAPI().
    postAsync('wps-mfw/get-membership-offers',
     request, 
    requireToken: false)
    
    );
  } else {
    return handleResponse(await MightyAPI().postAsync('wps-mfw/get-membership-offers', request, requireToken: false));
  }
}


Future checkMembership(request) async {
  if (!await isGuestUser() && await isLoggedIn()) {
    return handleResponse(await
     MightyAPI().
    postAsync('wps-mfw/get-user-membership',
     request, 
    requireToken: false)
    
    );
  } else {
    return handleResponse(await MightyAPI().postAsync('wps-mfw/get-user-membership', request, requireToken: false));
  }
}
...
Future addToCart(request) async {
  return handleResponse(await 
  MightyAPI().postAsync('store/api/v1/cart/add-cart/', request, requireToken: true));
}
